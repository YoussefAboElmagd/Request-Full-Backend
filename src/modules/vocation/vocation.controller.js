...
const getAllVocationsByCreatedBy = catchAsync(async (req, res, next) => {
  ...
  defaultQuery = await vocationModel
    .find()
    .limit(8)
    .select("nameEN _id createdBy");
...
const updateVocation = catchAsync(async (req, res) => {
  ...
  const updated = await vocationModel.findByIdAndUpdate(id, req.body, {
    new: true,
  });
  if (updated) {
...
const deleteVocation = catchAsync(async (req, res) => {
  ...
  const deleted = await vocationModel.findByIdAndDelete(
    id,
    { name },
    { new: true }
  );
